...
namespace SP4
{
    public class SynthPlusFour : Zplusnthbase
    {
        public SynthPlusFour()
        {
            SetWaveFormat(8000, 1);
        ...
        public override int Read(short[] buffer, int offset, int sampleCount)
        {
            for (int sample = 0; sample < sampleCount; sample++)
            {
                ...
                short csv = (short)currentsamplevalue;
                csv = (short)((csv / 8192) * 8192);
                buffer[sample + offset] = csv;
            }
            return sampleCount;
        }
